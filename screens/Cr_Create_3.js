...
const CreateStoryHeader = ({ title, content }) => {
    ...
    const handleFinalizeCreation = async () => {
        try {
            dispatch(setCreationField_3({
                chapterTitle: title,
                chapterContent: content,
            }));
            dispatch(initNewCreation());

            const { newCreation, newCreationChapter } = store.getState().account;

            dispatch(uploadNewCreationToFirestore({ newCreation, newCreationChapter }));

            dispatch(fetchCreationById(newCreation.bookId));
