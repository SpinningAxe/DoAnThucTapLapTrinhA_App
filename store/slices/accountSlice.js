...
export const loginUser = createAsyncThunk(
    ...
    async ({ email, password, onSuccess }, { rejectWithValue }) => {
        try {
            ...
            Alert.alert("Thành công", `Xin chào ${user?.name || user?.username || ""}!`);
...
export const TEMP_fetchTestAccount = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => { // No parameters needed
        try {
...
export const fetchCreationById = createAsyncThunk('account/fetchCreationById',
...
export const uploadNewCreationToFirestore = createAsyncThunk(
    ...
    async ({ newCreation, newCreationChapter }, { rejectWithValue }) => {
        try {
            ...
            return { bookId: newCreation.bookId, chapterId: newCreationChapter.chapterId };
        } catch (error) {
...
export const updateChapter = createAsyncThunk(
    ...
    async ({ updatedTitle, updatedContent, chapterToUpdate }, { rejectWithValue }) => {
        try {
            let updatedFields = {
                lastUpdateDate: Timestamp.now(),
            ...
            await updateDoc(doc(db, "Chapters", String(chapterToUpdate.id)), updatedFields);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchChaptersOfSelectedCreation.fulfilled, (state, action) => {
                state.loading = false;
                state.chaptersOfSelectedCreation = action.payload.sort((a, b) => a.chapterNum - b.chapterNum);;
            })
            ...
            .addCase(fetchChaptersOfCurrentBook.fulfilled, (state, action) => {
                state.loading = false;
                state.chaptersOfCurrentBook = action.payload.sort((a, b) => a.chapterNum - b.chapterNum);;
            })
            ...
            .addCase(removeBookFromLibrary.fulfilled, (state, action) => {
                state.loading = false;
                state.libraryBookIdList = state.libraryBookIdList.filter(bookId => bookId !== action.payload);
            })
